...
- **Reconsider if:** re-run the `HERD_RUSH_DAY = 10` + `HERD_RUSH_SIZE = 10` pair at eight seeds. It is the strongest thing measured since v12 on both gates at once and it is the first candidate to queue here. `FEED_DAYS_EARLY = 3` is now also screened as a *window* (extending it to day 12 or 16 is -$3,257) - the day-9 cash freeze it causes is real, $2,700 locked out of a $3,689 bank, but releasing it is not the fix. This also supersedes the two rejected feed-reserve entries below: the reserve *is* a throttle, but only in the opening, and cutting it season-long is noise.
...
- **Test again only with:** a shop rotation that changes which of milk/wool/egg the town drains. The "live price-sensitive species choice" this entry used to queue is now answered in the negative from the price model: the mix is already at the point where each species' marginal unit is worth about the same, which is what a fixed ratio is for.
