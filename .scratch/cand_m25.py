...
SHED_PRESSURE = 70
# Room kept clear for the goods the crew will still be holding at nightfall.
SHED_MARGIN = 25
...
def _market(me, priv, obs, load, n_animals, want, carried, grown):
    ...
    # The shed cap is enforced against what the *crew* is holding, not against
    # what is stored: `_drop_inventories_to_shed` empties every pack into the
    # shed at nightfall and silently discards the overflow. Measured on a seed-0
    # mirror at the competition's `townCenterSellInterval` of 24 the crew carries
    # 76-83 units at dusk against 51-71 units of room, and 174 units a game are
    # destroyed, 85 of them strawberry; the same game at the benchmark's interval
    # of 12 loses four, which is why this never showed up. So price the pressure
    # on the whole farm's stock rather than the shed's, and leave a margin,
    # because carried stock only grows between now and nightfall.
    pressure = (shed_used > SHED_PRESSURE
                or shed_used + sum(carried.values()) > SHED_CAP - SHED_MARGIN)
