...
def _fetch_jobs(tiles, invs, shed, wanted):
    ...
    for item, need in wanted.items():
        if item == "WHEAT":
            # Demand in *loaded carriers*, not in units. Sized off units the
            # instant one spends its wheat on a FEED it reopens a one-unit
            # shortfall and the whole crew tops itself back up.
            carried = sum(1 for inv in invs if inv.get(item, 0) > 0)
            need = min(need, MAX_CARRIERS)
        else:
