...
def _market(me, priv, obs, load, n_animals, want, carried, grown):
    ...
    # what is stored: `_drop_inventories_to_shed` empties every pack into it at
    # nightfall and discards the overflow. Measured on a seed-0 mirror at the
    # competition's `townCenterSellInterval` of 24, the crew carries 76-83 units
    # at dusk against 51-71 units of room and 174 units a game are destroyed, 85
    # of them strawberry; the same game at the benchmark's interval of 12 loses
    # four. So price the pressure on the whole farm's stock, not the shed's.
