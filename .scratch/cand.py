...
MAX_CARRIERS = 8
# Items a unit keeps when it walks to the shed: feed and livestock are on their
# way out, not back.
KEEP_ITEMS = frozenset(("WHEAT", "GOOSE", "COW", "SHEEP"))
# Produce in a pack this deep is worth the walk home before the day ends.
DROP_LOAD = 3
# Turn the day is over on; a unit starts its shed run with exactly enough turns
# left to arrive.
DUSK = 23
...
def _assign(units, jobs, tiles, invs, hour=0):
    ...
    # A pack is not a warehouse. Produce rides in a unit's inventory until that
    # unit has nothing else to do, and from day 19 this farm has no idle turns at
    # all, so every harvest waits for the nightly dump -- which discards whatever
    # does not fit. Measured on a seed-0 mirror at the competition's
    # `townCenterSellInterval` of 24: the crew carries 76-83 units at dusk
    # against 51-71 units of room in the shed, and 174 units a game are thrown
    # away, 85 of them strawberry -- about $13,000 a farm. The same game at the
    # benchmark's interval of 12 discards four, which is why this never showed up.
    # Send the loaded unit home at exactly the hour it can still reach a dock:
    # the ~596 moves a game that a crew spends walking at dusk toward jobs the
    # day ends before it reaches are deleted anyway (`_end_of_day` respawns every
    # unit on the NW dock), so those are the cheapest turns on the farm to spend.
    haul = set()
    for i in list(idle):
        if not docks or i >= len(invs):
            continue
        if sum(n for item, n in invs[i].items()
               if item not in KEEP_ITEMS and n > 0) < DROP_LOAD:
            continue
        reach = min(abs(units[i][0] - s[0]) + abs(units[i][1] - s[1]) for s in docks)
        if hour + reach >= DUSK:
            haul.add(i)
            idle.discard(i)
    ...
    for i in haul | idle:
        if not docks or i >= len(invs):
            continue
        if any(n > 0 for item, n in invs[i].items() if item not in KEEP_ITEMS):
            ux, uy = units[i]
...
def agent(obs):
    ...
    acts = _assign(units, jobs, tiles, invs, obs["hour"])
